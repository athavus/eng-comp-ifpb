...
char Keypad_Scan(void) {
	...
	for (int l = 0; l < 4; l++) {
		// linha atual em LOW
		GPIO_Write_Pin(GPIOC, lines[l], GPIO_PIN_RESET);
		Delay_ms(20); // debounce

		// Verifica cada linha
		for (int c = 0; c < 4; c++) {
			if (!GPIO_Read_Pin(GPIOC, columns[c])) {
				Delay_ms(20); // debounce
				while (!GPIO_Read_Pin(GPIOC, columns[c])); // espera soltar

				GPIO_Write_Pin(GPIOC, lines[l], GPIO_PIN_SET);
